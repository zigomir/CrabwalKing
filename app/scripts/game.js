...
$(function() {
    ...
    imm_img.src = IMMORTAL;
    
    // abbath initial position
    abb_x = (canvas.width / 2) - (abb_width / 2);
    abb_y = canvas.height - abb_height - 9; // because 10px ground
...
function stage() {
    // poster
    ctx.drawImage(imm_img, canvas.width / 2 - imm_img.width / 2, 100);
...
function posterColorInvert() {
    var imageData = ctx.getImageData(canvas.width / 2 - imm_img.width / 2, 
                                     100, imm_img.width, imm_img.height);
    ...
    ctx.putImageData(imageData, canvas.width / 2 - imm_img.width / 2, 100);
}
    ...
    switch (event.keyCode) {
        ...
        case 40:  /* Down arrow was pressed */
            if (abb_img.src.endsWith(ABBATH_GUITAR)) {
                abb_img.src = ABBATH_GUITAR_SLAM;
            } else {
                abb_img.src = ABBATH_GUITAR;
            }
            break;
...
function walk() {
    if (abb_img.src.endsWith(ABBATH_GUITAR)) {
        abb_img.src = ABBATH_GUITAR_WALK;
    } else {
        abb_img.src = ABBATH_GUITAR;
    }
}
